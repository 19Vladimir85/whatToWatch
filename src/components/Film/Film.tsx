...
export const Film: React.FC<IFilmProp> = ({name, poster, year, rating, id, countries, genres, alternativeName, isSmall}) => {
const rate = getRateNumber(rating)
return <Link to={`/movie/${id}`}><div className={styles.film}>
    ...
    <div className={styles.film__card_description_item}>{name}</div>
    <div className={styles.film__card_description_item}>{alternativeName}</div>
    <div className={styles.film__card_description_item}>{year}</div>
    <div className={styles.film__card_description_item}>{countries?.map((el)=> el.name)}, {genres?.map((el)=> el.name).join(", ")}</div>
    </div>
