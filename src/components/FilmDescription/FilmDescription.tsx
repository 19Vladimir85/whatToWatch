...
import {ICountry, IFilm, IGenre} from "../../types/films"
...
return <div className={styles.film}>
    <img src={film.poster ? film.poster.url : noPoster} alt="" className={styles.film__poster}/>
    ...
    <FieldInfo name="Страна">{film.countries?.map((el:ICountry)=> el.name)}</FieldInfo>
    <FieldInfo name="Жанр">{film.genres?.map((el:IGenre)=>el.name)}</FieldInfo>
</div>
</div>
}